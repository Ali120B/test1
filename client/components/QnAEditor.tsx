    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {questions
          ...
          <div
            key={item.id}
            className="bg-card text-card-foreground border border-border rounded-xl overflow-hidden hover:shadow-xl hover:shadow-primary/10 transition-all duration-300 hover:-translate-y-1"
          >
            {editingId === item.id ? (
                ...
                <div className="flex items-start justify-between mb-4">
                  ...
                  <div className="flex flex-col gap-4 ml-4">
                    {!item.answers?.some(a => a.isOfficial) && (
                      <div className="bg-muted ring-1 ring-border rounded-lg p-4 w-64">
                        ...
                        <textarea
                          ...
                          className="w-full px-3 py-2 text-sm border border-border rounded-md bg-background focus:ring-1 focus:ring-primary outline-none transition-all mb-2"
                          rows={2}
                        />
                        <button
                          onClick={() => handleAddAnswer(item.id)}
                          disabled={!answerInputs[item.id]}
                          className="w-full flex items-center justify-center gap-1.5 px-3 py-1.5 bg-secondary text-secondary-foreground text-xs font-bold rounded-md hover:bg-secondary/90 disabled:opacity-50 transition-all border border-secondary/20 shadow-sm"
                        >
                          <Plus size={14} /> Submit Answer
                        </button>
                      </div>
                    )}
